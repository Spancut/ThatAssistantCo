...
- **Decision (2026-07-28):** the product owner designated `What you should do next.docx` as the governing implementation roadmap unless another document explicitly supersedes a section.
- **Follow-up:** retain the recommendation to convert/split the approved content into versioned Markdown, but this no longer blocks controlled implementation.

### OD-003 — Is the fixed technology stack accepted?

- **Decision (2026-07-28):** accepting the Word document as governing also accepts its fixed stack for controlled implementation, subject to its own requirement to verify current official documentation at the relevant ticket.
- **Follow-up:** record implementation choices as ADRs during T001 and validate current versions during T000/T002.
...
- **Decision (2026-07-28):** current repository documentation is authoritative for product intent, while the Word document governs implementation unless another document explicitly supersedes a section. This is option B.
- **Follow-up:** retain identical wording in source indexes and T001 governance files.
...
- **Blocked:** durable Part 11 versioning and synchronization; not controlled ticket execution.
