...
## 2026-07-24 — Textarea round-trips need normalizing before comparing "was this edited"

`approveOutputAction` decides `approved` vs `edited_and_approved` by comparing the reviewer's submitted subject/body against the model's original stored values. Two normalization gaps made an *untouched* draft look edited every time, caught by the live Founder Playwright test (not a hypothetical — it failed the same way on every real run until fixed): the review form's Zod schema trims the submitted values but the comparison used the untrimmed original, and — the bigger one — the HTML form-submission spec normalizes `<textarea>` line breaks to `\r\n` when serialized, so even byte-identical text arrives with different line endings than the model's `\n`-only JSON. Fixed by trimming both sides before comparing and normalizing `\r\n → \n` inside the body schema itself (`lib/validations/ai.ts`), not just in the one comparison call site — anything else that reads a submitted `<textarea>` value and compares it against a non-form-sourced string has the same exposure.

## 2026-07-24 — `approvals` has no `org_id`; RLS joins to `outputs` instead

The AI-workflow spec's column list for `approvals` didn't include `org_id`, unlike every other workspace-scoped table. Rather than denormalize one in for RLS convenience, its policies check membership via `exists (select 1 from outputs o where o.id = approvals.output_id and is_org_member(o.org_id, auth.uid()))` — still "the same org-scoped membership pattern," just reached through a join since the column isn't there. Verified live the same way as every direct-`org_id` table: cross-org reads and writes are rejected. An `approvals` row is also only ever created at the moment a human actually approves/rejects (never pre-created as `pending`), so `approver_id`/`status` don't need to tolerate a not-yet-decided state in practice, even though `pending` exists in the enum for a possible future "assign a reviewer" flow.

## 2026-07-24 — `outputs.structured_content` is mutable, not an immutable original

An edited-and-approved output overwrites `structured_content.subject`/`.body` (and `draft_content`) with the reviewer's final text, rather than preserving the model's original response untouched forever. The model's *original* wording is not the thing worth preserving once a human has approved something different — `workflow_runs.input_snapshot` already has everything needed to reconstruct what was asked and what context it had, and `approvals.edited_content` keeps the edited body as an audit trail of the change itself. Keeping `structured_content` "current" means the review page never needs a second, separate "what does this output actually say now" field.

## 2026-07-24 — Safety-policy screening lives inside the one prompt, not a second model call

`docs/ai-orchestration.md` describes screening requests for earnings/health/legal/financial claims, invented testimonials, etc. before running a workflow. Implementing that as a *separate* classifier call would make this a two-model-call workflow, which the spec for this milestone explicitly ruled out ("no multi-step/agentic workflows... get this one pattern right first"). The guidance is instead embedded directly in `lib/ai/prompts/draft-email.ts`'s single prompt, instructing the model to omit disallowed claims and lower `confidence_flag` accordingly rather than refuse outright. This is a deliberate simplification for the first workflow, not the intended long-term shape — a dedicated pre-flight check is a reasonable candidate for a future milestone once there's a second workflow to justify shared screening logic.
