...
| ARCH-003 | Durable workers handle retries/delays/background steps | Architecture; Part 11 §3 | PARTIALLY COMPLETE | T002 pins Inngest, serves the local handler, and registers a safe infrastructure function; workflow durability remains T020 | workflow event, job | T020 | Add domain worker contracts only with workflow foundation | T020 |
...
| ARCH-005 | Typed boundaries plus runtime validation | CLAUDE; Part 11 §2 | PARTIALLY COMPLETE | Strict TypeScript and T002 runtime environment validation pass; domain boundaries do not yet exist | command, event, AI result | Domain tickets | Add runtime schemas with each boundary | T010 onward |
...
| T002-R | Local/staging Supabase+Inngest, env validation, health | Part 11 §18 T002 | COMPLETE | `supabase/config.toml`, Inngest route/function, typed env guard, `/api/health`, scripts, tests, and local/staging docs; Docker-backed runtime evidence is explicitly blocked | environment | T000; Docker for live check | Run documented live checks once Docker is available | T003 |
| T003-R | CI, Sentry, correlation, privacy-safe logs | Part 11 §18 T003 | BLOCKED | CI, Sentry minimisation, UUID correlation IDs, safe logs, staging-only test route, and deterministic privacy tests are implemented; Vercel root cause is confirmed but provider correction/event evidence cannot proceed without authorised Sentry project access | log, trace | Authorised Sentry organisation/project, DSN, and event inspection | Clear the identified Vercel output override only after the complete access gate passes, then execute the documented staging event/absence check | T003 |
...
| OBS-001 | Logs/traces redact client content and correlate operations | Part 11 §15 | PARTIALLY COMPLETE | T003 allowlists logs/tags, strips Sentry request/user/content fields, propagates UUID correlation IDs, and tests fictional raw-content absence; external staging evidence remains | trace, log | Sentry staging access | Complete provider-side acceptance evidence | T003 |
