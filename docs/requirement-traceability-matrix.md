...
| T003-R | CI, Sentry, correlation, privacy-safe logs | Part 11 §18 T003 | PARTIALLY COMPLETE | Immutable-SHA CI, Sentry minimisation, UUID correlation IDs, safe logs, staging-only test route, and deterministic privacy tests are implemented; external staging event evidence is pending | log, trace | Authorised Sentry/staging access | Execute documented staging event/absence check | T003 |
